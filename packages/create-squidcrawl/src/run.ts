...
export async function run(
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        'Worker deployment finished, but create-squidcrawl could not parse Wrangler output. The worker may still have been deployed. Check the streamed deploy logs above for details.',
        {
          cause: error,
        },
      );
