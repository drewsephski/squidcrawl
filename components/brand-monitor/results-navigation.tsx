...
import { LayoutGrid, MessageSquare, Trophy, Target, ArrowLeft, Menu, X } from 'lucide-react';
...
export function ResultsNavigation({
  ...
  const tabs = [
    { id: 'matrix' as ResultsTab, label: 'Matrix', fullLabel: 'Comparison Matrix', icon: LayoutGrid, color: '#6366f1' },
    { id: 'prompts' as ResultsTab, label: 'Prompts', fullLabel: 'Prompts & Responses', icon: MessageSquare, color: '#22d3ee' },
    { id: 'rankings' as ResultsTab, label: 'Rankings', fullLabel: 'Provider Rankings', icon: Trophy, color: '#f59e0b' },
    { id: 'visibility' as ResultsTab, label: 'Visibility', fullLabel: 'Visibility Score', icon: Target, color: '#10b981' },
  ];

  return (
    <>
      {/* Desktop Sidebar Navigation */}
      <nav className={`w-full lg:w-72 flex-shrink-0 flex flex-col h-auto lg:h-[calc(100vh-8rem)] lg:sticky lg:top-8 transition-all duration-700 ${
      ...
      <div className="lg:hidden w-full mb-4">
        <div className="flex items-center justify-between mb-3">
          ...
          {brandData && (
            <div className="flex items-center gap-2">
              <span className="text-xs text-[#71717a]">Visibility</span>
              <span className="text-sm font-bold text-[#f59e0b]">{brandData.visibilityScore}%</span>
            </div>
          )}
        </div>
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                key={tab.id}
                onClick={() => handleTabClick(tab.id)}
                className={`flex-shrink-0 flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold transition-all duration-300 whitespace-nowrap ${
                  isActive
                    ? 'bg-[#6366f1]/20 text-[#fafafa] border border-[#6366f1]/40'
                    : 'bg-[#12121a] text-[#a1a1aa] border border-[#2a2a3a] hover:border-[#3f3f46]'
                }`}
              >
                <Icon className={`w-3.5 h-3.5 ${isActive ? 'text-[#6366f1]' : 'text-[#71717a]'}`} />
                <span>{tab.label}</span>
              </button>
            );
          })}
        ...
        <button
          onClick={onRestart}
          className="mt-2 w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-xs font-semibold bg-[#12121a] text-[#a1a1aa] border border-[#2a2a3a] hover:border-[#3f3f46] transition-all duration-300"
        >
