...
If you want the quickest Cloudflare deployment path, start with
[Deploy with npm create squidcrawl](/docs/reference/self-hosting/create-squidcrawl).
It handles the API Worker setup for you in one guided flow.

When the guided flow finishes it shows the deployed Worker URL, a link back to
this doc, and the paths to `apps/workers/v0/.dev.vars` plus
`apps/workers/v0/.dev.vars.production` where the CLI stored your `JWT_SECRET`.
It also asks `Try your API now?` to run a quick test that auto-mints a 15-minute
JWT (if needed) and prints status, preview, and a reusable `curl` command without
saving any response files.
...
- Access to the project repo: [https://github.com/drewsephski/squidcrawl](https://github.com/drewsephski/squidcrawl)
...
`npm create squidcrawl` also writes your `JWT_SECRET` into
`apps/workers/v0/.dev.vars` and `apps/workers/v0/.dev.vars.production` so you can
reuse those values when you run Wrangler manually later.
