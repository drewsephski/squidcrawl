---
title: Deploy with npm create squidcrawl
description: Launch your own Squidcrawl API Worker on Cloudflare with one command.
---

`npm create squidcrawl` is the fastest way to launch your own Squidcrawl API on
...
Before you run `npm create squidcrawl`, make sure you have:
...
```bash
npm create squidcrawl
```

Want a safe preview first? Run `npm create squidcrawl -- --dry-run` to
rehearse the full setup before you deploy.

You can also pass the target folder up front:

```bash
npm create squidcrawl ../my-app
```
...
2. Clone the Squidcrawl template
...
[the `npm create squidcrawl` docs](https://squidcrawl.dev/docs/reference/self-hosting/create-squidcrawl),
