...
`readUrl` is Squidcrawl’s full-fidelity page fetcher. It returns structured metadata, cleaned/markdown/HTML variants, optional metrics, robots data, and more—perfect for agents that need the entire page context in one call.


## Real screenshot of `readUrl` from playground
![Read URL example](../../assets/read-url-example.png)

## When to use this endpoint
- You need metadata, cleaned HTML, markdown, or robots manifest as `JSON` in addition to plain text.
- You want Squidcrawl to capture metrics and logging info for debugging or analytics.
...
curl \
  ...
  -X POST "https://api.squidcrawl.dev/read" \
  -d '{
