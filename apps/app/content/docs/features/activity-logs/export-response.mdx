...
- `GET https://api.squidcrawl.dev/logs/export?id={requestId}&format={format}`
...
```bash
curl "https://api.squidcrawl.dev/logs/export?id=log_456&format=markdown" \
...
- `format=markdown`
  ...
  Squidcrawl extracted this markdown from https://example.com.
  ```
