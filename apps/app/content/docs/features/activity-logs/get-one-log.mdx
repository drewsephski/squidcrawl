...
debug exactly what Squidcrawl processed.


## Endpoint

- `GET https://api.squidcrawl.dev/logs?id={requestId}`
...
- `id` *(string, required)* — Request/response identifier returned by any Squidcrawl operation.

## Example Request

```bash
curl "https://api.squidcrawl.dev/logs?id=log_123" \
