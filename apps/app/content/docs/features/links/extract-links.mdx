...
Link extraction works by parsing the actual HTML content of your target page—no sitemap.xml, robots.txt, or other configuration files needed. Squidcrawl intelligently discovers links by analyzing the page structure, making it work on any website regardless of their SEO setup.
</Callout>


## How does `Links Tree` look like in real usage?

This abbreviated snapshot comes from a real crawl of `hono.dev`, if you are logged into the dashboard already [click this url](https://api.squidcrawl.dev/links?url=hono.dev) from your browser to see the raw response, or you can try it out from [here in playground](http://squidcrawl.dev/app?op=extractLinks&url=hono.dev).
...
curl \
  ...
  -X POST "https://api.squidcrawl.dev/links" \
  -d '{
...
const tree = await squidcrawl.extractLinks('https://example.com', {
  ...extractLinksOptions,
});
...
<Callout type="info" title="ExtractLinksResponse">

- `ExtractLinksResponseWithTree`
  <AutoTypeTable
    path="../../packages/contracts/src/links.ts"
    name="ExtractLinksResponseWithTree"
  />
- `ExtractLinksResponseWithoutTree`
  <AutoTypeTable
    path="../../packages/contracts/src/links.ts"
    name="ExtractLinksResponseWithoutTree"
  />
