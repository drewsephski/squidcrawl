---
title: Node.js SDK
description: Install and use the official Squidcrawl JavaScript/TypeScript SDK client in Node runtimes.
---

Squidcrawl’s SDK wraps every API endpoint with typed methods, retry helpers, and structured errors. Use it anywhere you can run server-side JavaScript—Next.js route handlers, serverless functions, edge workers (Node-compatible), CLI tools, or background jobs.

## Requirements
- Node.js 18.17+ (Node 20+ recommended for built-in `fetch` and `AbortController`).
- Access to a Squidcrawl API key (see [Quick Start](/docs/overview/quick-start)).
...
<Steps>
  ...
  <Step>
    Optional: declare a custom `DEEPCRAWL_API_URL` if you self-host the workers. Leave it unset to use `https://api.squidcrawl.dev`.
  </Step>
  <Step>
...
export async function getMarkdown(url: string) {
  const markdown = await squidcrawl.getMarkdown(url);
...
export async function extractLinks(url: string) {
  const result = await squidcrawl.extractLinks(url);
...
export async function safeGetMarkdown(url: string) {
  try {
    return await squidcrawl.getMarkdown(url);
  } catch (error) {
    if (error instanceof DeepcrawlAuthError) {
      // Missing/invalid API key
      throw new Error('Check Squidcrawl credentials');
    }
...
If you want provide `squidcrawl` as your service in your system, you can use the following examples.
...
app.post('/api/read', async (req, res) => {
  try {
    const result = await squidcrawl.readUrl({
      url: req.body.url,
      metadata: true,
    });
    res.json(result);
...
<AutoTypeTable
  path="../../packages/sdks/js-ts/src/_types/index.ts"
  name="DeepcrawlConfig"
/>
...
<AutoTypeTable
  path="../../packages/sdks/js-ts/src/_types/index.ts"
  name="DeepcrawlFetchOptions"
/>

## Base Hosts & Authentication

- **Production host**: `https://api.squidcrawl.dev`.
- **Self-hosted Worker**: pass `baseUrl` to `new DeepcrawlApp({ baseUrl })` or
  send requests directly to your Worker origin.
- **Authentication**: send `Authorization: Bearer <DEEPCRAWL_API_KEY>` or
  `x-api-key: <DEEPCRAWL_API_KEY>`. Dashboard sessions may also forward signed
  cookies; prefer API keys for automation.
- **Content types**: POST endpoints expect JSON (UTF-8); GET endpoints accept
  query parameters that map to the option types below.

### Rate limits & retries

- Workers return `429` with code `RATE_LIMITED` and a `retryAfter` duration
  (seconds).
- The SDK automatically retries idempotent operations (`getMarkdown`, `readUrl`,
  `extractLinks`) with exponential backoff unless the error is explicitly typed.
