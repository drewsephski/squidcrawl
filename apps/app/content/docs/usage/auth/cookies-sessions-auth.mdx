---
title: Cookie Sessions Auth
description: Use the Squidcrawl dashboard login to call the API with secure session cookies directly from your browser.
---

Squidcrawl uses [better-auth](https://better-auth.com/) to power dashboard sessions. When you sign in at <Link href="https://squidcrawl.dev">squidcrawl.dev</Link>, the same session cookies can authorize direct requests to `https://api.squidcrawl.dev`—no API key required. This is ideal for quick tests, manual QA, or low-friction internal tools that run inside your browser.

## How cookie auth works
- The dashboard issues HTTP-only session cookies scoped to `.squidcrawl.dev`. They contain signed tokens; only Squidcrawl’s backend can read them.
- When you visit `https://api.squidcrawl.dev/read?url=…` in the same browser, the cookies travel with the request, and the middleware (`cookieAuthMiddleware`) resolves your session automatically.
...
<Callout type="info">
Cookie sessions only work when your dashboard and API are under the same parent
domain (for example, `app.example.com` and `api.example.com`). If your
deployment uses different apex domains (common with free `*.vercel.app` +
`*.workers.dev`), use API keys (the dashboard may create a device-scoped
`PLAYGROUND_API_KEY` for its own API calls).
</Callout>
...
<Steps>
  ...
  <Step>
    Open a new tab and visit <code>https://api.squidcrawl.dev/read?url=hono.dev</code> (extremely fast). The response delivers markdown directly (
    <code>text/markdown; charset=utf-8</code>).
  </Step>
  <Step>
    Switch to <code>https://api.squidcrawl.dev/links?url=hono.dev</code> or other endpoints as needed. Session cookies authorize GET and POST requests from the same browser.
  </Step>
</Steps>
...
const url = 'https://api.squidcrawl.dev/read?url=https://example.com';
