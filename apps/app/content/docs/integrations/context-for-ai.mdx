...
description: Map any site with Squidcrawl extractLinks so agents fetch only what matters.
---

Few modern sites publish an `llms.txt` manifest, so most AI agents still crawl blindly or pull entire HTML pages into memory. Squidcrawl's `extractLinks` endpoint skips the guesswork by returning a live hierarchy of every reachable page without relying on static sitemaps or hand-curated lists.

## Why `extractLinks` unlocks native web context
- **No dependency on `sitemap.xml` or `llms.txt`.** Squidcrawl walks the public navigation the same way a browser does, so single-page apps and hybrid renderers expose their real structure automatically.
...
2. **Return the link tree from an API route.** Cap depth and link counts so you stay within the context window you want to spend.
   ...
   export async function GET() {
     const result = await squidcrawl.extractLinks({
...
3. **Let the agent decide what to fetch.** Pull the tree first, then target the exact pages that matter before escalating to heavier endpoints like `readUrl`.
   ...
   export async function fetchPageSummary(url: string) {
     const { tree } = await squidcrawl.extractLinks({ url, tree: true, depth: 1 });
     ...
     const page = await squidcrawl.readUrl({
       url: docsLink.url,
       markdown: true,
       metadata: true,
     });
