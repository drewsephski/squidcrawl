...
description: Build a Next.js 16 agent with ai-sdk Agent and Squidcrawl schemas.
---

This walkthrough assumes a **Next.js 16 App Router** project using the latest ai-sdk `Agent` class. The agent will:

- Map a domain with Squidcrawl `extractLinks`.
...
The example targets `https://hono.dev` (the framework powering Squidcrawl's backend), but you can change the URL per request.
...
## 2. Tools using Squidcrawl schemas
...
export const extractLinks = tool({
  description: 'Build a site map for a domain using Squidcrawl extractLinks.',
  parameters: ExtractLinksOptionsSchema,
  async execute({ url, tree, ...options }) {
    const response = await squidcrawl.extractLinks({
      url,
      tree: tree ?? true,
      ...options,
    });
...
export const getMarkdown = tool({
  description:
    'Convert a public page into clean markdown using Squidcrawl getMarkdown.',
...
export const readUrl = tool({
  description:
    'Fetch metadata, markdown, and optional cleaned HTML via Squidcrawl readUrl.',
  parameters: ReadUrlOptionsSchema,
  async execute({ url, ...options }) {
    const response = await squidcrawl.readUrl({
      url,
...
<Callout type="info">Need to limit what the model can tweak? Swap `ExtractLinksOptionsSchema` (or the other schemas) for a `.pick(...)` subset. This example feeds the full schema so the agent has complete control.</Callout>
...
export const squidcrawlResearchAgent = new Agent({
...
export async function POST(request: Request) {
  ...
  return squidcrawlResearchAgent.respond({
...
- **Retry strategy**: Squidcrawl already retries transient failures; avoid wrapping tools with extra retry loops unless required.
