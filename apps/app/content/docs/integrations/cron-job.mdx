---
title: Cron Job
description: How to use Cron Job with Squidcrawl.
---

Cron jobs let you run Squidcrawl tasks on a schedule so you can keep data fresh without touching the dashboard.
...
- `squidcrawl` npm package installed (`pnpm add squidcrawl`).

## Step-by-step: schedule a Squidcrawl task
...
2. **Create a shared Squidcrawl client.** Keep the client server-only so your API key never reaches the browser.
...
3. **Handle the cron request in an API route.** This handler crawls a docs page, extracts the markdown content and metadata to watch for content changes over time.
   ...
   export async function GET() {
     ...
     console.log('Squidcrawl content snapshot', {
